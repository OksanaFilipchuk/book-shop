...
function showPageContent(data) {
  ...
  function displayCartContent() {
    if (cart.length > 0) {
      cart.forEach((el) => {
        ...
        bookBlock.innerHTML = `<img src = "${el.imageLink}" alt = "${
          el.title
        }" class = "cart-book-img">
  ...
  cartWrapper.addEventListener("drop", (event) => {
    ...
    if (cart.includes(data[el])) {
      cart[cart.indexOf(data[el])].number =
        cart[cart.indexOf(data[el])].number + 1;
    } else {
      data[el].number = 1;
      cart.push(data[el]);
    }
...
function updateCartSum() {
  if (cart.length === 0) {
    ...
    cartSum = cart.reduce(
      (accum, current) => accum + current.price * current.number,
      0
    );
  }
