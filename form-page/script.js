...
let fields = [
  ...
  },
  {
    ...
    validCondision: function (event) {
      return (
        event.target.value.length < 5 ||
        !/^[a-zA-Z()0-9]+$/.test(event.target.value)
      );
    },
  },
  {
...
giftInputs.forEach((el) =>
  el.addEventListener("change", () => {
    let checkedGiftValue = Array.from(giftInputs).reduce(
      (ac, curr) => ac + (curr.checked ? 1 : 0),
      0
    );
    if (checkedGiftValue === 2) {
      Array.from(giftInputs).forEach((el) => {
        if (!el.checked) el.setAttribute("disabled", "disabled");
      });
    } else {
      Array.from(giftInputs).forEach((el) => {
        if (!el.checked) el.removeAttribute("disabled");
      });
    }
  })
);
...
document.querySelector(".form").addEventListener("submit", (e) => {
  ...
  wrapper.innerHTML =
    "<p class = 'customer-info-header'>Summarized information</p>";
  fields.forEach((el) => {
